...
function renderSidebarHtml(nodes, level = 0) {
    ...
    nodes.forEach((node) => {
        ...
        // 🚀 关键：必须保留 nav-level-${level}，否则 CSS 颜色和缩进全废
        ...
        if (!node.url || node.url.includes("javascript:void(0)")) {
            html += `            <span class="nav-text ${node.hasChildren ? 'folder-text' : ''}" ${node.hasChildren ? 'onclick="toggleNode(this.previousElementSibling)"' : ''}>${node.text}</span>\n`;
        } else {
            html += `            <a href="#${hash}" onclick="handleManualClick(this)">${node.text}</a>\n`;
        }
        html += `        </div>\n`;
        if (node.hasChildren && node.children) html += renderSidebarHtml(node.children, level + 1);
        html += `    </li>\n`;
    });
    return html + "</ul>\n";
}
...
(async () => {
    ...
    console.clear();
    console.log("============================================================");
    console.log("🚀 [启动指引] V100 并发满血 + 目录秒读版");
        ...
        while (currentIndex < navItems.length) {
            ...
            try {
                ...
                if (rawCss) {
                    ...
                    if (!seenCssHashes.has(cssHash)) {
                        seenCssHashes.add(cssHash);
                        globalStyles.push(rawCss);
                    }
                }
    ...
    const offlineFixCss = ``;
    const uiCss = `<style>
        body { display: flex; height: 100vh; margin: 0; overflow: hidden; font-family: "Segoe UI", sans-serif; color: #333; }
        .nx-sidebar { width: 340px; min-width: 250px; overflow-y: auto; padding: 15px 5px; background: #f8f9fa; border-right: 1px solid #dee2e6; flex-shrink: 0; }
        .resizer { width: 5px; cursor: col-resize; background: #dee2e6; flex-shrink: 0; }
        .main-content { flex: 1; overflow-y: auto; scroll-behavior: smooth; background: #fff; }
        .content-wrapper { max-width: 1200px; margin: 0 auto; padding: 40px; }

        .nx-sidebar ul { list-style: none !important; margin: 0; padding: 0; }
        .nav-item-row { display: flex; align-items: flex-start; margin: 2px 0; }
        ul.nested { display: none; padding-left: 4px; border-left: 1px solid #888; margin-left: 7px; }
        ul.active { display: block; }

        /* 🚀 目录分级颜色对齐 Python 版 */
        .nx-sidebar a, .nav-text { text-decoration: none !important; color: #005f87 !important; padding: 3px 6px; border-radius: 4px; font-size: 13px; }
        .nav-level-0 > .nav-item-row > a, .nav-level-0 > .nav-item-row > span.nav-text { font-size: 14px; font-weight: 700; color: #00557d !important; }
        .selected-link { background: #005a84 !important; color: #fff !important; }
        
        .caret { cursor: pointer; display: inline-block; width: 14px; text-align: center; margin-top: 6px; }
        .caret::before { content: "▶"; font-size: 10px; transition: transform 0.2s; display: inline-block; }
    ...
    <div class="nx-sidebar">
        ...
        <div>${renderSidebarHtml(treeData)}</div>
    </div>
