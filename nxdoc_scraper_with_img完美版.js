...
async function capturePageContent(page) {
          ...
          Array.from(videos).map((v) => {
            return new Promise((resolve) => {
              ...
              const checkRealSrc = () => {
                ...
                // 额外检查：是否有 data-real-src 属性
                const videoEl = v.querySelector("video");
                if (videoEl && videoEl.getAttribute("data-real-src")) {
                  return true;
                }
                return false;
              };
              ...
              // 动态轮询：每 500ms 检查一次，最多等待 15 秒
              let attempts = 0;
              const maxAttempts = 30; // 15 秒 / 500ms = 30 次

              const pollInterval = setInterval(() => {
                attempts++;
                if (checkRealSrc() || attempts >= maxAttempts) {
  ...
  return await frame
    .evaluate(async () => {
      ...
      // 🌟 再次在 clone 上识别小图标（Base64 转换后尺寸可能变化）
...
async function startJob(sub, mode) {
  ...
  async function worker(id) {
    ...
    const createPage = async () => {
      ...
      // 拦截无关请求（字体、埋点、广告等），放行 xhr/fetch/media
        if (
    ...
    while (currentIndex < navItems.length && !isShuttingDown) {
      ...
      while (retries < 3 && !success && !isShuttingDown) {
        try {
          ...
          if (data.html) {
            // 智能判断：检测页面是否包含视频组件但没有获取到真实链接
            const hasVideoComponent = data.html.includes('disw-video') || data.html.includes('<video');
            const hasRealVideoLink = data.html.includes('data-real-src="http') || data.html.includes("data-real-src='http");
            // 如果有视频组件但没有真实链接，说明抓取不完整，需要重试
            if (hasVideoComponent && !hasRealVideoLink && retries < 2) {
              console.log(
                `   ⚠️ [${item.text}] [线程${id}] 检测到视频链接未获取到，准备重试...`,
              );
              throw new Error("视频真实链接未获取到，需要重试");
            }
            ...
            // 主动内存回收，每处理 50 个页面重启一次 Context
            if (localSuccessCount % 50 === 0) {
              console.log(`[线程${id}] ♻️ 触发内存回收，正在重启浏览器上下文...`);
              await createPage();
            }
          } else {
            throw new Error("HTML 为空");
          }
        } catch (e) {
          retries++;
          console.error(
